...
...                 AND    create and schedule landing page  ${landing_page["button_add"]["data_sync"]["insert_and_update_type"]}
...
landing_page_properties
    ...
    element should be visible    ${landing_page["button_add"]["general_properties"]["landing_page_url"]}

preview_landing_page
    open content    ${landing_page}    ${landing_page["button_list"]["list"]}
    wait until keyword succeeds    5x    1 sec    click element    ${landing_page["button_list"]["first_row"]}
    click element    ${landing_page["button_list"]["preview"]}
    wait until keyword succeeds    5x    1 sec    select window    title=Preview: Untitled
