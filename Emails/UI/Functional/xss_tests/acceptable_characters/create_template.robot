*** Settings ***
Documentation       will check the input of unicode values into the template fields
...
template_name
    ...
    wait until element is visible     ${template["button_list"]["table"]}
    ...
    ${is_equal}=    run keyword and return status    should not be equal as integers    ${post_test_template_id}    ${most_recent_template_id}
    run keyword unless    ${is_equal}    fail    msg=No new templates were created with non-XSS data
