*** Settings ***
Documentation       will check the input of unicode values into the create sms message builder fields
...
*** Test Cases ***
sms_message_search
    verify non xss data on search field    ${sms_message_builder}    ${sms_message_builder["button_list"]["list"]}

sms_name
    # Get first item of sms message table
    open content    ${sms_message_builder}    ${sms_message_builder["button_list"]["list"]}
    wait until element is visible     ${sms_message_builder["button_list"]["table"]}
    ...
    ${is_equal}=    run keyword and return status    should not be equal as integers    ${post_test_sms_message_id}    ${most_recent_sms_message_id}
    run keyword unless    ${is_equal}    fail    msg=No new sms messages were created with non-XSS data
...
sms_rename_name
    ...
    run keyword if    ${is_failed}    fail    msg=No new sms messages were created with non-XSS data
