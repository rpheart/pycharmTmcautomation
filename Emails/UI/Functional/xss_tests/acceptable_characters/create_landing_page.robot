*** Settings ***
Documentation       will check the input of unicode values into the create landing page search screen
...
*** Test Cases ***
landing_page_search
    verify non xss data on search field    ${landing_page}    ${landing_page["button_list"]["list"]}
...
landing_page_name_save
    ...
    run keyword unless    ${is_equal}    fail    msg=No new Landing Pages were created with non-XSS data
...
landing_page_rename_name
    ...
    run keyword if    ${is_failed}    fail    msg=No new Landing Pages were created with non-XSS data
