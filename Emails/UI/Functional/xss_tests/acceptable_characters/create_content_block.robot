*** Settings ***
Documentation       will check the input of unicode values into the Content Block screen
...
content_block_name
    ...
    ${is_equal}=    run keyword and return status    should not be equal as integers    ${post_test_content_block_id}    ${latest_content_block_id}
    run keyword unless    ${is_equal}    fail    msg=No new Content Blocks were created with non-XSS data
...
content_block_search
    verify non xss data on search field    ${content_block}    ${content_block["button_list"]["list"]}
...
content_block_rename_name
    ...
    run keyword if    ${is_failed}    fail    msg=No new Content Blocks were created with non-XSS data
