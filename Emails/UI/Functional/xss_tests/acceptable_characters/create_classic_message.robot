*** Settings ***
Documentation       will check the input of unicode values into the create classic message builder screen
...
classic_message_search
    verify non xss data on search field    ${classic_message_builder}    ${classic_message_builder["button_list"]["list"]}
...
message_name
    ...
    ${is_equal}=    run keyword and return status    should not be equal as integers    ${post_test_classic_message_id}    ${most_recent_classic_message_id}
    run keyword unless    ${is_equal}    fail    msg=No new classic messages were created with non-XSS data
