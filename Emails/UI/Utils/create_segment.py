...
def open_connection(https, response_status):
	try:
		...
		if response.status_code == int(response_status):
			...
			return ("total response is {}".format(response.status_code),"{}".format(token_string[left_string*4-3:right_string]))
		else:
...
def create_segment(https, email_status):
	try:
		...
		if message.status_code == int(email_status) and check_message_string.find('success')!= -1:
			...
			return ("{}".format(id[id_left*4-5:id_right]),"total response is {}".format(message.status_code),"{}".format('success'))
