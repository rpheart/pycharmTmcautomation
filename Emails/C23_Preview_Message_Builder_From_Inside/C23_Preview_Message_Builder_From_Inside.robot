...
Preview_New_Message_Builder_From_Inside
    Login_Message_Cloud_Click_Email    # 1.Login to message cloud 2.Click on Email tab
...
Login_Message_Cloud_Click_Email
...
Verify_Message_Displayed_Is_Same_As_Build
    ...
    Select Frame    id=emv-ccmd-iframe    # Click on inner frame
    Sleep    10
    Element Should Contain    xpath=//*[@id="tabledivColumn-0-0"]/div    Preview New Message Builder From Inside
...
Select_Tests_Enter_Details_Click_Preview_Message
    ...
    #Unselect Frame
    #Select Frame    id=preview-iframe    # Select preview frame
    run keyword and ignore error    Wait until element is visible    css=body > table > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td > h2    timeout=10
