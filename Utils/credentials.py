dev = {
    "ui_server": "https://qa-sfui.themessagecloud.com",
    "ccmd": "https://p2itgcce.campaigncommander.com",
    "ui_username": "sidev_qa",
    "ui_password": "computer2.",
    "ui_liveensure_username": "sidev_live_qa",
    "ui_liveensure_password": "computer2.",
    "api_server": "p2itgapie.emv2.com",
    "api_user": "sidev_qa_api",
    "api_password": "computer1.",
    "api_key": "CdX7Cr1V5FWKh39MUf0uq5PtSzUOacvD-wjtSiVihvVpJzE",
    "marketing_verified_email": "email@sidev01.cccampaigns.net",
    "trans_verified_email": "email@sidev01.nmp1.com",
}

qa = {
    "ui_server": "https://qa-sfui.themessagecloud.com",
    "ccmd": "https://p2itgcce.campaigncommander.com",
    "ui_username": "emvqa_qa",
    "ui_password": "computer4.",
    "ui_liveensure_username": "emvqa_live_qa",
    "ui_liveensure_password": "computer3.",
    "api_server": "p1itgapie.emv2.com",
    "api_user": "emvqa_qa_api",
    "api_password": "qa11focus$",
    "api_key": "CdX7CrxD4EeekH9MUf0uq5PtXTEcfdysjzntS4SNd1hmLkbF",
    "marketing_verified_email": "test@emvqae1.ccemails.net",
    "trans_verified_email": "email@emvqae1.nmp1.com",
}

preprod = {
    "ui_server": "https://preprod.themessagecloud.com",
    "ccmd": "https://p0cce.campaigncommander.com",
    "ui_username": "preprod_qa",
    "ui_password": "computer3.",
    "ui_liveensure_username": "preprod_live_qa",
    "ui_liveensure_password": "computer2.",
    "api_server": "p0apie.emv3.com",
    "api_user": "preprod_qa_api",
    "api_password": "qa11focus$",
    "api_key": "CdX7CrBW_1Sfg09Zb9MuhJuXcXYaftijmEepH9RFaPDoulZS9hGSRg",
    "marketing_verified_email": "email@preprodtmc.ccemails.net",
    "trans_verified_email": "email@preprodtmc.nmp1.com",
}

pod_e1 = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "qa_qaaute1",
    "ui_password": "computer2.",
    "ui_liveensure_username": "live_qaaute1",
    "ui_liveensure_password": "computer1.",
    "api_server": "emvapi.emv3.com",
    "api_user": "qa_qaaute1_api",
    "api_password": "computer1.",
    "api_key": "CdX7Cr9X7G6ekEFIRMd-hJuXcXYbbdymnk3cS5kdXOdGOU-WAg",
    "marketing_verified_email": "email@qaaute1.ccemails.net",
    "trans_verified_email": "email@qaaute1.nmp1.com",
}

pod_e1_ops = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "auto_e1",
    "ui_password": "computer4.",
    "ui_liveensure_username": "auto_e1_live_qa",
    "ui_liveensure_password": "computer4.",
    "api_server": "emvapi.emv3.com",
    "api_user": "auto_e1_api",
    "api_password": "qa11focus$",
    "api_key": "CdX7CrxH-EWArkUMb8M_svCiVSoFfM6sjzntS5he2qAcrkZu",
    "marketing_verified_email": "email@emvops-e1.cccampaigns.net",
    "trans_verified_email": "email@emvops-e1.nmp1.com",
}

pod_e2 = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "qa_qaaute2",
    "ui_password": "computer2.",
    "api_server": "p2apie.emv3.com",
    "api_user": "qa_qaaute2_api",
    "api_password": "computer1.",
    "api_key": "CdX7Cr9X7G6ekEFIRMd9hJuXcXYbbdymnk3fS5kdXOdGOU-WAA",
    "marketing_verified_email": "email@qaaute2.ccemails.net",
    "trans_verified_email": "email@qaaute2.nmp1.com",
}

pod_e2_ops = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "auto_e2",
    "ui_password": "computer4.",
    "api_server": "p2apie.emv3.com",
    "api_user": "auto_e2_api",
    "api_password": "qa11focus$",
    "api_key": "CdX7CrxH-EWArkUPb8M_svCiVSoFfM6sjzrtS5tJyJeyLkdL",
    "marketing_verified_email": "email@emvops-e2.cccampaigns.net",
    "trans_verified_email": "email@emvops-e2.nmp1.com",
}

pod_e3 = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "qa_qaaute3",
    "ui_password": "computer2.",
    "api_server": "p3apie.emv3.com",
    "api_user": "qa_qaaute3_api",
    "api_password": "computer1.",
    "api_key": "CdX7Cr9X7G6ekEFIRMd8hJuXcXYbbdymnk3eS5kdXOdGOU-WDg",
    "marketing_verified_email": "email@qaaute3.ccemails.net",
    "trans_verified_email": "email@qaaute3.nmp1.com",
}

pod_e3_ops = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "auto_e3",
    "ui_password": "computer4.",
    "api_server": "p3apie.emv3.com",
    "api_user": "auto_e3_api",
    "api_password": "qa11focus$",
    "api_key": "CdX7CrxH-EWArkUOb8M_svCiVSoFfM6sjzvtS5hhq3zIrkbi",
    "marketing_verified_email": "email@emvops-e3.cccampaigns.net",
    "trans_verified_email": "email@emvops-e3.nmp1.com",
}

pod_e4 = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "qa_qaaute4",
    "ui_password": "computer2.",
    "api_server": "p4apie.emv3.com",
    "api_user": "qa_qaaute4_api",
    "api_password": "computer1.",
    "api_key": "CdX7Cr9X7G6ekEFIRMd7hJuXcXYbbdymnk3ZS5kdXOdGOU-WDA",
    "marketing_verified_email": "email@qaaute4.ccemails.net	",
    "trans_verified_email": "email@qaaute4.nmp1.com",
}

pod_e4_ops = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "auto_e4",
    "ui_password": "computer4.",
    "api_server": "p4apie.emv3.com",
    "api_user": "auto_e4_api",
    "api_password": "qa11focus$",
    "api_key": "CdX7CrxH-EWArkUJb8M_svCiVSoFfM6sjzztS5hfOQbwrkb-",
    "marketing_verified_email": "email@emvops-e4.cccampaigns.net",
    "trans_verified_email": "email@emvops-e4.nmp1.com",
}

pod_e5 = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "qa_qaaute5",
    "ui_password": "computer1.",
    "api_server": "p5apie.emv3.com",
    "api_user": "qa_qaaute5_api",
    "api_password": "computer1.",
    "api_key": "CdX7Cr9X7G6ekEFIRMd6hJuXcXYbbdymnk3YS5kdXOdGOU-WCg",
    "marketing_verified_email": "email@qaaute5.ccemails.net",
    "trans_verified_email": "email@qaaute5.nmp1.com",
}

pod_e5_ops = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "auto_e5",
    "ui_password": "computer4.",
    "api_server": "p5apie.emv3.com",
    "api_user": "auto_e5_api",
    "api_password": "qa11focus$",
    "api_key": "CdX7CrFH-EWArkUIb8M_svCiVSoFfM6smkepDqwGaPLJxOBPR5qD",
    "marketing_verified_email": "email@techopsteam.emv9.net",
    "trans_verified_email": "email@emvops_pode5.nmp1.com",
}

pod_e6 = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "qa_qaaute6",
    "ui_password": "computer1.",
    "api_server": "p6apie.emv3.com",
    "api_user": "qa_qaaute6_api",
    "api_password": "computer1.",
    "api_key": "CdX7Cr9X7G6ekEFIRMd5hJuXcXYbbdymnk3bS5kdXOdGOU-WCA",
    "marketing_verified_email": "email@qaaute6.ccemails.net",
    "trans_verified_email": "email@qaaute6.nmp1.com",
}

pod_e6_ops = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "auto_e6",
    "ui_password": "computer4.",
    "api_server": "p6apie.emv3.com",
    "api_user": "auto_e6_api",
    "api_password": "qa11focus$",
    "api_key": "CdX7CrxH-EWArkULb8M_svCiVSoFfM6sjz7tS5h06anyLkcM",
    "marketing_verified_email": "email@emvops-e6.cccampaigns.net",
    "trans_verified_email": "email@emvops-e6.nmp1.com",
}

pod_e7 = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "qa_qaaute7",
    "ui_password": "computer1.",
    "api_server": "p7apie.emv3.com",
    "api_user": "qa_qaaute7_api",
    "api_password": "computer1.",
    "api_key": "CdX7Cr9X7G6ekEFIRMd4hJuXcXYbbdymnk3aS5kH0y44kU-UCA",
    "marketing_verified_email": "email@qaaute7.ccemails.net",
    "trans_verified_email": "email@qaaute7.nmp1.com",
}

pod_e7_ops = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "auto_e7",
    "ui_password": "computer4.",
    "api_server": "p7apie.emv3.com",
    "api_user": "auto_e7_api",
    "api_password": "qa11focus$",
    "api_key": "CdX7CrxH-EWArkUKb8M_svCiVSoFfM6sjz_tS5hhq3zIrkYa",
    "marketing_verified_email": "email@emvops-e7.cccampaigns.net",
    "trans_verified_email": "email@emvops-e7.nmp1.com",
}

pod_e8 = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "qa_qaaute8",
    "ui_password": "computer1.",
    "api_server": "p8apie.emv3.com",
    "api_user": "qa_qaaute8_api",
    "api_password": "computer1.",
    "api_key": "CdX7Cr9X7G6ekEFIRMd3hJuXcXYbbdymnk3VS5kH0y44kU-UFg",
    "marketing_verified_email": "email@qaaute8.ccemails.net",
    "trans_verified_email": "email@qaaute8.nmp1.com",
}

pod_e8_ops = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "auto_e8",
    "ui_password": "computer4.",
    "api_server": "p8apie.emv3.com",
    "api_user": "auto_e8_api",
    "api_password": "qa11focus$",
    "api_key": "CdX7CrxH-EWArkUFb8M_svCiVSoFfM6sjzDtS5jBk83srkYt",
    "marketing_verified_email": "email@emvops-e8.cccampaigns.net",
    "trans_verified_email": "email@emvops-e8.nmp1.com",
}

pod_e9 = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "qa_qaaute9",
    "ui_password": "computer1.",
    "api_server": "p9apie.emv3.com",
    "api_user": "qa_qaaute9_api",
    "api_password": "computer1.",
    "api_key": "CdX7Cr9X7G6ekEFIRMd2hJuXcXYbbdymnk3US5kH0YXko8-XNQ",
    "marketing_verified_email": "email@qaaute9.ccemails.net",
    "trans_verified_email": "email@qaaute9.nmp1.com",
}

pod_e9_ops = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "auto_e9",
    "ui_password": "computer4.",
    "api_server": "p9apie.emv3.com",
    "api_user": "auto_e9_api",
    "api_password": "qa11focus$",
    "api_key": "CdX7CrxH-EWArkUEb8M_svCiVSoFfM6sjzHtS5iSdgJkrkZ8",
    "marketing_verified_email": "email@emvops-e9.ccemails.net",
    "trans_verified_email": "email@emvops-e9.nmp1.com",
}

pod_t1 = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "qa_qaautt1",
    "ui_password": "computer2.",
    "api_server": "emvapi.emv2.com",
    "api_user": "qa_qaautt1_api",
    "api_password": "computer1.",
    "api_key": "CdX7Cr9X7G6ekEFIRNZ-hJuXcXYbbdymnlzcS5kH0y44kU-UJg",
    "marketing_verified_email": "email@qaautt1.ccemails.com",
    "trans_verified_email": "email@qaautt1.nmp1.com",
}

pod_t1_ops = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "auto_t1",
    "ui_password": "computer4.",
    "api_server": "emvapi.emv2.com",
    "api_user": "auto_t1_api",
    "api_password": "qa11focus$",
    "api_key": "CdX7CrxH-EWArlQMb8M_svCiVSoFfM6snjntS5hfOQbwrkbm",
    "marketing_verified_email": "email@emvops-t1.cccampaigns.com",
    "trans_verified_email": "email@emvops-t1.nmp1.com",
}

pod_t2 = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "qa_qaautt2",
    "ui_password": "computer2.",
    "api_server": "p2apic.emv2.com",
    "api_user": "qa_qaautt2_api",
    "api_password": "computer1.",
    "api_key": "CdX7Cr9X7G6ekEFIRNZ9hJuXcXYbbdymnlzfS5kH0y44kU-UJA",
    "marketing_verified_email": "email@qaautt2.ccemails.com",
    "trans_verified_email": "email@qaautt2.nmp1.com",
}

pod_t2_ops = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "auto_t2",
    "ui_password": "computer4.",
    "api_server": "p2apic.emv2.com",
    "api_user": "auto_t2_api",
    "api_password": "qa11focus$",
    "api_key": "CdX7CrxH-EWArlQPb8M_svCiVSoFfM6snjrtS5hfOQbwrkbk",
    "marketing_verified_email": "email@emvops-t2.cccampaigns.com",
    "trans_verified_email": "email@emvops-t2.nmp1.com",
}

pod_t3 = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "qa_qaautt3",
    "ui_password": "computer2.",
    "api_server": "p3apic.emv2.com",
    "api_user": "qa_qaautt3_api",
    "api_password": "computer1.",
    "api_key": "CdX7Cr9X7G6ekEFIRNZ8hJuXcXYbbdymnlzeS5kH0y44kU-UIg",
    "marketing_verified_email": "email@qaautt3.ccemails.com",
    "trans_verified_email": "email@qaautt3.nmp1.com",
}

pod_t3_ops = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "auto_t3",
    "ui_password": "computer4.",
    "api_server": "p3apic.emv2.com",
    "api_user": "auto_t3_api",
    "api_password": "qa11focus$",
    "api_key": "CdX7CrxH-EWArlQOb8M_svCiVSoFfM6snjvtS5iSdgJkrkZq",
    "marketing_verified_email": "email@emvops-t3.cccampaigns.com",
    "trans_verified_email": "email@emvops-t3.nmp1.com",
}

pod_t4 = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "qa_qaautt4",
    "ui_password": "computer2.",
    "api_server": "p4apic.emv2.com",
    "api_user": "qa_qaautt4_api",
    "api_password": "computer1.",
    "api_key": "CdX7Cr9X7G6ekEFIRNZ7hJuXcXYbbdymnlzZS5kH0y44kU-UIA",
    "marketing_verified_email": "email@qaautt4.ccemails.com",
    "trans_verified_email": "email@qaautt4.nmp1.com",
}

pod_t4_ops = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "auto_t4",
    "ui_password": "computer4.",
    "api_server": "p4apic.emv2.com",
    "api_user": "auto_t4_api",
    "api_password": "qa11focus$",
    "api_key": "CdX7CrxH-EWArlQJb8M_svCiVSoFfM6snjztS5hcAP1orkaL",
    "marketing_verified_email": "email@emvops-t4.cccampaigns.com",
    "trans_verified_email": "email@emvops-t4.nmp1.com",
}

pod_t5 = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "qa_qaautt5",
    "ui_password": "computer2.",
    "api_server": "p5apic.emv2.com",
    "api_user": "qa_qaautt5_api",
    "api_password": "computer1.",
    "api_key": "CdX7Cr9X7G6ekEFIRNZ6hJuXcXYbbdymnlzYS5kHuEAOnU-UHQ",
    "marketing_verified_email": "email@qaautt5.ccemails.com",
    "trans_verified_email": "email@qaautt5.nmp1.com",
}

pod_t5_ops = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "auto_t5",
    "ui_password": "computer4.",
    "api_server": "p5apic.emv2.com",
    "api_user": "auto_t5_api",
    "api_password": "qa11focus$",
    "api_key": "CdX7CrxH-EWArlQIb8M_svCiVSoFfM6snj3tS5h2-91grkYe",
    "marketing_verified_email": "email@emvops-t5.cccampaigns.com",
    "trans_verified_email": "email@emvops-t5.nmp1.com",
}

pod_t6 = {
    "ui_server": "https://www.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "prod_qaautt6",
    "ui_password": "computer1.",
    "api_server": "p6apic.emv2.com",
    "api_user": "qa_qaautt6_api",
    "api_password": "computer1.",
    "api_key": "CdX7Cr9X7G6ekEFIRNZ5hJuXcXYbbdymnlzbS5kHrVoj7U-UeQ",
    "marketing_verified_email": "email@qaautt6.ccemails.com",
    "trans_verified_email": "email@qaautt6.nmp1.com",
}

pod_t6_ops = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "auto_t6",
    "ui_password": "computer4.",
    "api_server": "p6apic.emv2.com",
    "api_user": "auto_t6_api",
    "api_password": "computer2.",
    "api_key": "CdX7CrxH-EWArlQLb8M_svCiVSoFfM6snj7tS5hrCNOeLkcu",
    "marketing_verified_email": "email@emvops-t6.ccemails.com",
    "trans_verified_email": "email@emvops-t6.nmp1.com",
}

production = {
    "ui_server": "https://sfui.themessagecloud.com",
    "ccmd": "https://www.campaigncommander.com",
    "ui_username": "",
    "ui_password": "",
    "api_server": "",
    "api_user": "",
    "api_password": "",
    "api_key": ""
}
