...
def send_requests():
    ...
    request_list = [
        ...
        api.offer_click(click, guid, engagement, email=email, lid=lid, idx=idx, position="", timestamp=one_day_past)
    ]
...
class TestOfferOpenEmailAndOfferClickEmailAndLoginWithNewCookie(unittest.TestCase):
    ...
    def test_browse_cbtt_contains_all_event_information(self):
        self.assertTrue(utils.verify_json_contains_events(filtered_response[3]),
                        msg="browse cbtt event is missing this campaign information")
